...
// ---------------------------------------------------------------------------
// Per-statement verb suggestion (Workflow B fallback / Workflow A rules).
//
// The model suggests a verb per dimension/participant inline (see classify.js);
// this deterministic mapper is the fallback when the model omits one and the
// sole source in offline (local) mode. It only ever returns IRIs that exist in
// the VERBS catalog above, so results are always valid.
// ---------------------------------------------------------------------------
...
// Ordered keyword → verb rules for rubric dimension names. First match wins.
const DIMENSION_VERB_RULES = [
  [/question|inquir|curios/i, V.asked],
  [/scal|rating|\brate/i, V.rated],
  [/review|feedback|reflect/i, V.reviewed],
  [/contract|listen|safety|presence|rapport|relationship/i, V.talkedWith],
  [/goal|option|action|account|commit|\bplan|will|solution|problem|forward|deepen|know-how|resource/i, V.shared],
];

export function suggestStatementVerb({ kind = "overall", name = "", scaleType = "categorical", context = "" } = {}) {
  if (kind === "participant") {
    const hay = `${name} ${context}`;
    if (/\bask|question/i.test(hay)) return V.asked;
    if (/\banswer|respond|replied/i.test(hay)) return V.answered;
    return V.commented;
  }
  if (kind === "dimension") {
    const rule = DIMENSION_VERB_RULES.find(([re]) => re.test(name));
    return rule ? rule[1] : V.evaluated;
  }
  // overall evaluation statement
  return scaleType === "numeric" ? V.rated : V.evaluated;
}
